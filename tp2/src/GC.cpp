...
int GradientConjugue(MatVirt &A, MatVirt &C,
                     ...
                     int niveauimpression) {   // niveauimpression: 0: pas impression .. 10 a chaque iteration
    int n = A.n, nret = 0;
    double NaN = nan("");
    ...
    if (gCg < 1e-30) {
        ...
        for (int iter = 1; iter <= nbitermax; ++iter) {
            gCgp = gCg;
            double HAH = mysdot(n, H, ProduitMatVec(A, H, AH));
//            std::cout << " " << HAH << std::endl;
            rho = -mysdot(n, G, H) / HAH;
            mysaxpy(n, rho, H, x);
            mysaxpy(n, rho, AH, G);
            gCg = mysdot(n, G, ProduitMatVec(C, G, CG));
            gamma = gCg / gCgp;
...
int GradientConjugue(MatVirt &A,
                     double *b, // second membre
                     double *x, // solution qui contient une initialisation
                     int nbitermax, double eps,
                     int niveauimpression) {   // niveauimpression: 0: pas impression .. 10 a chaque iteration

    MatIdentite conditionerIdentity(A.n);
    return GradientConjugue(A, conditionerIdentity, b, x, nbitermax, eps, niveauimpression);
}
